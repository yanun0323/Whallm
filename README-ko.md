...
도구는 클라이언트가 실행하고 결과를 서버에 돌려줍니다. 이미지, 오디오, `logprobs`, `response_format`, `stop`은 지원하지 않습니다. 요청 본문은 **1 MiB**로 제한됩니다.
...
지정된 세 가지 텍스트 모델 체크포인트만 지원합니다. 새 가속 경로는 소규모 모델과 구성 요소 단위로 테스트했으며, 전체 모델의 속도 및 품질 비교는 아직 완료되지 않았습니다. 매우 긴 프롬프트에는 더 많은 캐시 메모리가 필요합니다.
