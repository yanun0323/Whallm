...
ツールはクライアント側で実行し、結果をサーバーに返します。画像、音声、`logprobs`、`response_format`、`stop` は非対応です。リクエスト本文の上限は **1 MiB** です。
...
対応するのは、指定された 3 つのテキストモデルのチェックポイントのみです。新しい高速化処理は小規模モデルと部品単位でテストしていますが、実際のモデルでの速度と品質の比較は未完了です。長いプロンプトには、より多くのキャッシュメモリが必要です。
