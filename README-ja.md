...
開発中のソースでは、すべてのモデルで `/v1/responses` の `function_call_output.output` に文字列または `input_text` の配列を指定できます。テキストは順番に連結され、既存の空白と改行は保持されます。区切り文字は追加しません。リクエストの履歴には、同じ `call_id` を持つ `function_call` を含めてください。メディアの対応範囲は引き続きモデルによって異なります。この修正は v1.1.8 には含まれていません。
